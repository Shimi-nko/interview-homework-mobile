...
describe('root endpoint - create', () => {
  ...
  it('shouldn`t create product if there are non supported values in body', async () => {
    const product = {
      ...MOCK_DATA[0],
...
it('shouldn`t update product if there are non supported values in body', async () => {
  const createProductRes = await app.request('', {
    method: 'POST',
    body: JSON.stringify(MOCK_DATA[0]),
  });
...
describe('root endpoint - delete', () => {
  ...
  it('shouldn`t delete product if there are non supported values in body', async () => {
